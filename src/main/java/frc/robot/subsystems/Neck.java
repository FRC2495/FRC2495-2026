...
public class Neck extends SubsystemBase implements INeck {
	...
	public static final double ANGLE_TO_TRAVEL_REVS_ABS_ENC = 0.31; // we divide by ticks per revolution to convert the ticks unit to revolutions
	...
	*/
	...
	static final double MOVE_PROPORTIONAL_GAIN = 5.0; // An error of 1 rotation results in 3.0 V output
	...
	*/	
	...
	public void joystickControl(Joystick joystick)
	{
		...
		{
			neck.setControl(neckReducedOut.withOutput(+joystick.getY()*0.1));
		}
	}	
	...
	public void gamepadControl(XboxController gamepad)
	{
		if (!isMoving) // if we are already doing a move we don't take over
		{
			neck.setControl(neckReducedOut.withOutput(MathUtil.applyDeadband(-gamepad.getLeftY(),RobotContainer.GAMEPAD_AXIS_THRESHOLD)*0.1)); // adjust sign if desired
		}
	}
